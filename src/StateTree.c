...
void fixTreeTime(TypeTree *tree) {
	...
	if(tree->time == NULL) {
		...
		if(tree->time[tree->root] == NO_TIME)
			tree->time[tree->root] = 0.0;
		for(n=0; n<tree->size && tree->time[n] != NO_TIME; n++)
			;
		if(n<tree->size) {
			int m;
			for(m=0; m<tree->size; m++)
				tree->time[m] = 1.;
			fprintf(stderr, "Warning! Node %d has no time. All branchlengths are replaced by 1.\n", n);
		}
	}
...
int testLeaves(TypeTree *tree) {
	...
	for(n=0; n<tree->size; n++)
		if(isUnknown(n, tree) && tree->node[n].child == -1) {
			printf("Leaf index %d with unknown state\n", n);
...
void readStateItem(FILE *f, TypeStateItem **item, int *nitem) {
	...
	*nitem = 0;
	do {
		char *tmp;
		int i;
		tmp = (char*) monmalloc((MAX_NAME_SIZE+1)*sizeof(char));
		skipSeparator(f);
		c = fgetc(f);
		if(c == '\'' || c == '"') {
			c = fgetc(f);
		...
		if(i>1) {
			...
			(*item)[*nitem].name = (char *) monrealloc((void*) tmp, i*sizeof(char));
			skipSeparator(f);
			c = fgetc(f);
			for(i=0; i<MAX_NAME_SIZE && c !=EOF && !IsSeparator(c); i++) {
				bof[i] = c;
			...
			if(i == MAX_NAME_SIZE)
				exitProg(ErrorExec, "Number too much long...");;
			bof[i++] = '\0';
...
fprintLexiTree(stdout, dict);
	for(i=0; i<list->size; i++) {
		int n;
printf("%s\t%d\n", list->name[i], findWordLexi(list->name[i], dict));
